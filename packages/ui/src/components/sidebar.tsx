...
import {
  ...
  SheetHeader,
  SheetTitle,
} from "@mindorbit/ui/components/sheet"
import { Skeleton } from "@mindorbit/ui/components/skeleton"
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "@mindorbit/ui/components/tooltip"
import { useIsMobile } from "@mindorbit/ui/hooks/use-mobile"
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:after:bg-sidebar-border cubic-bezier(0.4, 0, 0.2, 1) absolute inset-y-0 z-20 hidden w-4 transition-all duration-200 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-1/2 after:inset-y-0 after:w-[2px] sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "no-scrollbar flex min-h-0 flex-1 flex-col gap-0 overflow-auto transition-all duration-300 ease-in-out group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
